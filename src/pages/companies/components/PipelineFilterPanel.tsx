...
interface PipelineFilterPanelProps {
  isOpen: boolean;
  onClose: () => void;
  onApply: (filters: PipelineFiltersState) => void;
  initialFilters: PipelineFiltersState;
  anchorRef: React.RefObject<HTMLButtonElement | HTMLDivElement>;
  jobId: number;
}

type TabKey = keyof PipelineFiltersState;
...
export default function PipelineFilterPanel({
  ...
  const [filters, setFilters] = useState<PipelineFiltersState>(initialFilters);
    ...
    () =>
      debounce(async (query: string, jId: number) => {
        try {
          ...
          const suggestions = await jobPostService.getDesignationList(jId, query);
  ...
  const getFilterCount = (tab: TabKey) => {
    ...
    if (tab === "attention") return filters.attention.length;
  ...
  const renderListOptions = (options: FilterOption[], isSearching: boolean, activeListKey: "location" | "designation" | "noticePeriod" | "attention") => {
    const currentListValues = activeListKey === "noticePeriod" 
      ? filters.noticePeriod.selected 
      : activeListKey === "attention" ? filters.attention : activeListKey === "location" ? filters.location : filters.designation;
  ...
  const renderContent = () => {
    ...
    if (activeTab === "attention") {
      return (
        <div className="flex-1 flex flex-col p-6 bg-white">
          <div className="flex justify-between items-center mb-6">
            <h3 className="text-base font-semibold text-[#0F47F2]">Attention Required</h3>
          </div>
          {renderListOptions(ATTENTION_OPTIONS, false, "attention")}
        </div>
      );
    }
