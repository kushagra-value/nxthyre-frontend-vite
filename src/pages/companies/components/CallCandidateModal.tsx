        ...
        <div className="fixed inset-0 z-[60] flex items-center justify-center bg-slate-900/40 backdrop-blur-sm p-6">
            <div className="bg-white w-full max-w-2xl rounded-2xl shadow-2xl border border-slate-200 overflow-hidden flex flex-col">
                ...
                <div className="bg-[#1D4ED8] p-8 pb-10 relative flex flex-col items-center justify-center">
                    ...
                    <h2 className="text-white text-2xl font-semibold mb-1">{candidate.name}</h2>
                ...
                <div className="p-8">
                    {step === 1 ? (
                        <div className="flex flex-col items-center animate-in fade-in duration-300">
                            <h3 className="text-slate-700 text-lg mb-6">Ready to call this candidate?</h3>
                            ...
                            <div className="bg-slate-50 rounded-xl p-6 w-full grid grid-cols-2 gap-y-6 gap-x-4 border border-slate-100">
                                <div>
                                    <p className="text-slate-400 text-xs mb-1">Year of Experience</p>
                                    <p className="text-slate-700 font-medium text-sm">{candidate.experience}</p>
                                </div>
                                <div>
                                    <p className="text-slate-400 text-xs mb-1">Expected - CTC</p>
                                    <p className="text-slate-700 font-medium text-sm">{candidate.expectedCtc}</p>
                                </div>
                                <div>
                                    <p className="text-slate-400 text-xs mb-1">Location</p>
                                    <p className="text-slate-700 font-medium text-sm">{candidate.location}</p>
                                </div>
                                <div>
                                    <p className="text-slate-400 text-xs mb-1">Notice Period</p>
                                    <p className="text-slate-700 font-medium text-sm">{candidate.noticePeriod}</p>
                                </div>
                            </div>
                        </div>
                    ) : (
                        <div className="flex flex-col h-full animate-in fade-in slide-in-from-bottom-2 duration-300">
                            ...
                            <div className="sticky bottom-0 left-0 right-0 pt-4 mt-2 bg-white border-t border-slate-100 flex gap-4">
                                ...
                                <button
                                    ...
                                    className="flex-[2] py-3 rounded-lg bg-[#1D4ED8] text-white font-bold text-sm hover:bg-blue-700 transition-colors shadow-lg shadow-blue-200"
                                >
                                    Schedule Follow-up
                                </button>
                            </div>
