...
export interface NaukbotFiltersState {
  location: string[];
...
interface NaukbotFilterPanelProps {
  isOpen: boolean;
  onClose: () => void;
  onApply: (filters: NaukbotFiltersState) => void;
  initialFilters: NaukbotFiltersState;
  anchorRef: React.RefObject<HTMLButtonElement | HTMLDivElement>;
  jobId: number;
}

type TabKey = keyof NaukbotFiltersState;
...
export default function NaukbotFilterPanel({
  isOpen,
  onClose,
  onApply,
  initialFilters,
  anchorRef,
  jobId,
}: NaukbotFilterPanelProps) {
  const [activeTab, setActiveTab] = useState<TabKey>("location");
  const [filters, setFilters] = useState<NaukbotFiltersState>(initialFilters);
  ...
  const [jobRoleSuggestions, setJobRoleSuggestions] = useState<FilterOption[]>([]);
  const [isSearchingJobRole, setIsSearchingJobRole] = useState(false);
  ...
  // Debounced API fetch for jobRole (using designation service)
  const fetchJobRoleSuggestions = useMemo(
    () =>
      debounce(async (query: string, jId: number) => {
        try {
          setIsSearchingJobRole(true);
          const suggestions = await linkedinBotService.getDesignationList(jId, query);
          setJobRoleSuggestions(suggestions.map((s: string) => ({ value: s, label: s })));
        } catch (error) {
          console.error("Failed to fetch job role suggestions", error);
        } finally {
          setIsSearchingJobRole(false);
        }
      }, 300),
    []
  );
  ...
  useEffect(() => {
    ...
    if (activeTab === "jobRole" && isOpen) {
      fetchJobRoleSuggestions(searchQuery, jobId);
    }
  }, [searchQuery, activeTab, fetchLocationSuggestions, fetchJobRoleSuggestions, jobId, isOpen]);
  ...
  const handleReset = () => {
    setFilters(EMPTY_NAUKBOT_FILTERS);
  };
  ...
  const getFilterCount = (tab: TabKey) => {
    ...
    if (tab === "jobRole") return filters.jobRole.length;
  ...
  const renderListOptions = (options: FilterOption[], isSearching: boolean, activeListKey: "location" | "jobRole" | "noticePeriod" | "skills") => {
    const currentListValues = activeListKey === "noticePeriod" 
      ? filters.noticePeriod.selected 
      : activeListKey === "skills" ? filters.skills : activeListKey === "location" ? filters.location : filters.jobRole;

    const isApiDriven = activeListKey === "location" || activeListKey === "jobRole";
  ...
  const renderContent = () => {
    ...
    const showSearchBar = ["location", "jobRole"].includes(activeTab);
    const searchPlaceholder = activeTab === "location" ? "Search locations..." : "Search job roles...";
    const options = activeTab === "location" ? locationSuggestions : jobRoleSuggestions;
    const isSearching = activeTab === "location" ? isSearchingLocation : isSearchingJobRole;
    ...
    return (
      <div className="flex-1 flex flex-col p-6 bg-white">
        ...
        {renderListOptions(options, isSearching, activeTab as "location" | "jobRole")}
      </div>
    );
  };
