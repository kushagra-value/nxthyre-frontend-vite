...
import { ArrowLeft, User, Mail, Phone, Copy, Calendar, Award, Briefcase, GraduationCap, MessageCircle, X, Send, Star, TrendingUp, FileText, CheckCircle, Clock, AlertCircle, Users, Target, BarChart3, ChevronDown, Edit, Archive, Trash2 } from 'lucide-react';
...
const PipelineStages: React.FC<PipelineStagesProps> = ({ onBack, activeTab, setActiveTab }) => {
  ...
  const renderStageDetails = () => {
    if (!selectedCandidate) {
      return (
        <div className="text-center text-gray-500 mt-8">
          ...
          <p className="text-sm mt-1">Select a candidate from the list to view their details</p>
        </div>
      );
    }
    ...
    switch (selectedStage) {
            ...
            <div className="bg-gray-50 rounded-lg p-3 space-y-2">
              <div className="flex justify-between items-center">
                <div className="flex items-center space-x-2">
                  ...
                  <span className="text-sm text-gray-700 truncate">{selectedCandidate.email}</span>
                </div>
              ...
              <div className="flex justify-between items-center">
                <div className="flex items-center space-x-2">
                  ...
                  <span className="text-sm text-gray-700">{selectedCandidate.phone.number}</span>
                </div>
                <div className="flex space-x-2">
                  <FontAwesomeIcon icon={faWhatsapp} className="w-4 h-4 text-gray-400 hover:text-green-600 cursor-pointer" />
                  <Copy className="w-4 h-4 text-gray-400 hover:text-gray-600 cursor-pointer" />
                </div>
              </div>
            </div>
            ...
            <div>
              ...
              {selectedCandidate.positions.map((pos, index) => (
                <div key={index} className="text-sm text-gray-700">
                  ...
                  <p className="text-gray-600">{pos.companyName} | {pos.location}</p>
                </div>
              ))}
            </div>
          ...
          <div className="space-y-4">
            <div className="bg-blue-50 rounded-lg p-3">
              ...
              <div className="space-y-1 text-sm">
                <p><span className="font-medium">Date Sent:</span> {inviteData?.dateSent}</p>
                <p><span className="font-medium">Status:</span> 
                  <span className={`ml-1 px-2 py-1 rounded-full text-xs ${
                    inviteData?.responseStatus === 'Interested' ? 'bg-green-100 text-green-800' : 'bg-yellow-100 text-yellow-800'
                  }`}>
                    {inviteData?.responseStatus}
                  </span>
                </p>
                <p><span className="font-medium">Current Status:</span> {inviteData?.currentStatus}</p>
              </div>
            </div>
        ...
        return (
          <div className="space-y-4">
            <div className="bg-green-50 rounded-lg p-3">
              <h4 className="font-medium text-green-900 mb-2">Application Details</h4>
              <div className="space-y-1 text-sm">
                <p><span className="font-medium">Applied Date:</span> {appliedData?.appliedDate}</p>
                <p><span className="font-medium">Resume Score:</span> 
            ...
            <div className="grid grid-cols-2 gap-3">
              <div className="bg-gray-50 rounded-lg p-3">
                <h5 className="font-medium text-gray-900 text-sm mb-1">Skills Match</h5>
                <p className="text-lg font-bold text-blue-600">{appliedData?.skillsMatch}</p>
              </div>
              <div className="bg-gray-50 rounded-lg p-3">
                <h5 className="font-medium text-gray-900 text-sm mb-1">Experience Match</h5>
                <p className="text-lg font-bold text-green-600">{appliedData?.experienceMatch}</p>
              </div>
            </div>

            <div>
              <h4 className="font-medium text-gray-900 mb-2">Resume Highlights</h4>
              <div className="flex flex-wrap gap-2">
                {appliedData?.highlights?.split(', ').map((highlight, index) => (
      ...
      case 'AI Interview':
      case 'Shortlisted':
        const interviewData = selectedStage === 'AI Interview' ? stageData.aiInterview : stageData.shortlisted;
        return (
          <div className="space-y-4">
            <div className="bg-purple-50 rounded-lg p-3">
              <h4 className="font-medium text-purple-900 mb-2">Interview Results</h4>
              <p className="text-sm"><span className="font-medium">Date:</span> {interviewData?.interviewedDate}</p>
            </div>

            <div className="grid grid-cols-2 gap-2">
              <div className="bg-gray-50 rounded-lg p-2 text-center">
                <p className="text-xs text-gray-600">Resume Score</p>
                <p className="text-lg font-bold text-blue-600">{interviewData?.resumeScore}</p>
              </div>
              <div className="bg-gray-50 rounded-lg p-2 text-center">
                <p className="text-xs text-gray-600">Knowledge</p>
                <p className="text-lg font-bold text-green-600">{interviewData?.knowledgeScore}</p>
              </div>
              <div className="bg-gray-50 rounded-lg p-2 text-center">
                <p className="text-xs text-gray-600">Communication</p>
                <p className="text-lg font-bold text-yellow-600">{interviewData?.communicationScore}</p>
              </div>
              <div className="bg-gray-50 rounded-lg p-2 text-center">
                <p className="text-xs text-gray-600">Integrity</p>
                <p className="text-lg font-bold text-purple-600">{interviewData?.integrityScore}</p>
              </div>
            </div>

            <div className="bg-red-50 rounded-lg p-3">
              <h5 className="font-medium text-red-900 mb-2">Proctoring Check</h5>
              <div className="grid grid-cols-2 gap-2 text-xs">
                <div>Device Usage: {interviewData?.proctoring?.deviceUsage}%</div>
                <div>Assistance: {interviewData?.proctoring?.assistance}%</div>
                <div>Reference Material: {interviewData?.proctoring?.referenceMaterial}%</div>
                <div>Environment: {interviewData?.proctoring?.environment}%</div>
              </div>
            </div>

            <button className="w-full px-4 py-2 bg-purple-600 text-white rounded-lg hover:bg-purple-700 transition-colors">
              {selectedStage === 'AI Interview' ? 'Move to Shortlisted' : 'Schedule First Interview'}
            </button>
          </div>
        );
      ...
      case 'HR Round':
        const roundData = selectedStage === 'First Interview' ? stageData.firstInterview : 
                          selectedStage === 'Other Interviews' ? stageData.otherInterviews : stageData.hrRound;
        return (
          <div className="space-y-4">
            <div className="bg-indigo-50 rounded-lg p-3">
              <h4 className="font-medium text-indigo-900 mb-2">Interview Details</h4>
              <div className="space-y-1 text-sm">
                <p><span className="font-medium">Date:</span> {roundData?.interviewDate}</p>
                <p><span className="font-medium">Interviewer:</span> {roundData?.interviewerName}</p>
                <p><span className="font-medium">Email:</span> {roundData?.interviewerEmail}</p>
              </div>
            </div>
        ...
        return (
          <div className="space-y-4">
            <div className="bg-orange-50 rounded-lg p-3">
              <h4 className="font-medium text-orange-900 mb-2">Salary Details</h4>
              <div className="space-y-1 text-sm">
                <p><span className="font-medium">Salary Range:</span> {salaryData?.salary}</p>
                <p><span className="font-medium">Negotiation:</span> {salaryData?.negotiation}</p>
              </div>
            </div>
        ...
        return (
          <div className="space-y-4">
            <div className="bg-emerald-50 rounded-lg p-3">
              <h4 className="font-medium text-emerald-900 mb-2">Offer Details</h4>
              <div className="space-y-1 text-sm">
                <p><span className="font-medium">Sent Date:</span> {offerData?.offerSentDate}</p>
                <p><span className="font-medium">Status:</span> 
                  <span className={`ml-1 px-2 py-1 rounded-full text-xs ${
                    offerData?.offerAcceptanceStatus === 'Accepted' ? 'bg-green-100 text-green-800' : 'bg-yellow-100 text-yellow-800'
                  }`}>
      ...
      case 'Archives':
        ...
        return (
          <div className="space-y-4">
            <div className="bg-gray-50 rounded-lg p-3">
              <h4 className="font-medium text-gray-900 mb-2">Archive Details</h4>
              <div className="space-y-1 text-sm">
                <p><span className="font-medium">Reason:</span> {archiveData?.reason}</p>
                <p><span className="font-medium">Archived Date:</span> {archiveData?.archivedDate}</p>
              </div>
            </div>
            ...
            <div className="bg-red-50 border border-red-200 rounded-lg p-3">
              <p className="text-sm text-red-700">This candidate has been archived and is no longer active in the pipeline.</p>
            </div>
          </div>
        );
          ...
          <div className="lg:col-span-3 order-2 lg:order-1">
            <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-3">
              <div className="flex items-center space-x-2 cursor-pointer" onClick={onBack}>
                ...
                <h3 className="text-sm font-semibold text-gray-600 mb-4 mt-1">Back to Dashboard</h3>
              </div>
              <div className="space-y-2">
              ...
              <div className="divide-y divide-gray-200">
                {currentCandidates.length === 0 ? (
                  <div className="p-8 text-center text-gray-500">
                    ...
                    <p className="text-sm mt-1">Candidates will appear here when they reach this stage</p>
                  </div>
                ) : (
                      ...
                      <div className="flex items-center space-x-3">
                        ...
                        <div className="flex-1 min-w-0">
                          <div className="flex items-center justify-between">
                            <h3 className="text-base font-semibold text-gray-900">{candidate.fullName}</h3>
                            <span className="px-2 py-1 text-xs bg-green-100 text-green-800 rounded-full">
                              {selectedStage}
                            </span>
                          </div>
                          <p className="text-sm text-gray-600 mt-1">{candidate.headline}</p>
                          <p className="text-sm text-gray-500 mt-1">{candidate.location.city}, {candidate.location.country}</p>
                        </div>
                      </div>
                    </div>
                  ))
              ...
              {selectedCandidate ? (
                <>
                  ...
                  <div className="flex items-center space-x-3">
                    ...
                    <div>
                      <h2 className="text-lg font-bold text-gray-900">{selectedCandidate.fullName}</h2>
                      <p className="text-sm text-gray-600">{selectedCandidate.headline}</p>
                      <p className="text-sm text-gray-500">{selectedCandidate.location.city}, {selectedCandidate.location.country}</p>
                    </div>
                  </div>
